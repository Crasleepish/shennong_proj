...
const portfolioId = ref(null) // 必填；在相关接口调用前校验
...
async function loadHoldingsAndTargets(weightsData) {
  ...
  const holdingsRes = await fetch(`${baseUrl}/service/current_position/${encodeURIComponent(portfolioId.value)}`, {
    headers: { 'Authorization': authHeader }
  })
  if (!holdingsRes.ok) {
    const msg = await holdingsRes.text().catch(() => '')
    ElMessage.error(`获取当前持仓失败：${msg}`)
    return
  }
