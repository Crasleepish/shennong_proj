...
output_prefix = "portfolio_BM_B_M"
...
def backtest_strategy(start_date: str, end_date: str):
    ...
    for i, date in enumerate(prices.index):
        if date in rb_dates:
            ...
            logger.info("Big cap group count on %s: %d", rb_date.strftime("%Y-%m-%d"), len(large_cap_stocks))
            ...
            num_low = int(len(bm_sorted) * 0.3)
            num_high = int(len(bm_sorted) * 0.7)
            selected_stocks = bm_sorted.index[num_low:num_high]
            # 确保调仓日的价格无缺失或异常
