...
output_dir = r"./result"
output_prefix = "portofolio_OP_S_L"
    ...
    for i, date in enumerate(prices.index):
        if date in rb_dates:
            ...
            if os.path.exists(industry_cache):
                ...
                industry_avg_df = pd.read_csv(industry_cache, dtype={'industry': str, 'avg_profitability': 'float64', 'std_profitability': 'float64'})
            ...
            for code in cap_selected_stocks:
                ...
                std_profit = industry_std_profit[industry]
                op_ratios[code] = (net_profit - avg_profit) / std_profit
            if not op_ratios:
            ...
            num_selected = int(len(op_sorted) * 0.3)
            selected_stocks = op_sorted.index[:num_selected]
            # 确保调仓日的价格无缺失或异常
